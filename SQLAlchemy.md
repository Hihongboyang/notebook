...
ins 对象的compile()方法返回一个SQLComplier对象，该对象允许我们通过params属性访问随查询一起发送的实际参数。

```python
ins.compile().params
```

添加好数据之后，需要再将命令提交到数据库
...
插入语句作为独立的函数实现，这样的实现，可以很好的复用代码，可以根据需要调整它的参数。
